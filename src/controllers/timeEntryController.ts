...
import type { TimeEntryType, TimeEntryUpdate } from '../types/timeEntry.js'
import type { ChangeLogInput } from '../types/changeLog.js'

const VALID_TYPES: TimeEntryType[] = ['IN', 'OUT']
...
export async function createTimeEntry(
  ...
  try {
    const rawUserId =
      ...
      (req.body?.user_id as number | string | undefined)

    const type = req.body?.type as TimeEntryType | undefined
...
export async function updateTimeEntry(
  ...
  try {
    ...
    if (req.body?.type !== undefined) {
      const t = req.body.type as TimeEntryType
